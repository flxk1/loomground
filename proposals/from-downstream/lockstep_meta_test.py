...
def _root() -> Path:
    ...
    for up in here.parents:
        if (up / "conformance" / "manifest.json").is_file():
            return up
        if (up / "Loomground" / "conformance" / "manifest.json").is_file():
...
def _vectors() -> list[Path]:
    man = json.loads((ROOT / "conformance" / "manifest.json").read_text())
    return [ROOT / "conformance" / "vectors" / v["name"] for v in man["vectors"]]
...
def check_declaration_coverage() -> list[str]:
    corpus = _inputs()
    declared = [d["name"] for d in json.loads((ROOT / "vocabulary" / "declarations.json").read_text())]
    fails = []
...
def _grammar_keywords() -> set[str]:
    ebnf = (ROOT / "grammar" / "loomground.ebnf").read_text()
