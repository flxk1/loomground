...
| human | Person, named by a role, to whom a reserved token is referred | None (not graph-connected) |
...
| Delegation | Attribute on an authority cord | Binds delegate to act on behalf of a delegator | [OCAP] |
...
- **human** — a person named by a reservation's role (§6), to whom a reserved token is referred; not graph-connected. A role does not by itself confer authority.
...
The `party` field denotes the party currently responsible — the in-language governance-responsibility attribute. Where a guard ranges over `party` (§6) it denotes the party at the evaluating gate. **The language does not authenticate party assignments; binding a `party` to an authenticated identity is outside this specification, and the party-guard and the separation-of-duty distinctness check (§6 Quorum) carry their intended security property only relative to such an external binding.**
...
A policy graph is **well-formed** if and only if: every cord is a permitted endpoint pairing; the `pipe` relation is acyclic (its transitive closure irreflexive); the graph contains exactly one master (§3); every gate lies on a `pipe ∪ egress` path to the master (reachability — no inert or dangling gate); a gate that declares a required grade is a source gate (§6, §7.1); and every delegation binding satisfies the no-amplification invariant (§6). A graph that is not well-formed is ill-formed, and a conforming implementation MUST give it no effect (fail-closed).
...
- **Party** — each actor and gate bears a party; where the party differs across a boundary, the party currently responsible changes accordingly. Communication of that change is outside this specification.
...
**Delegation binding (on-behalf-of).** An authority cord (§5.1) MAY declare that the actor (the *delegate*) acts on behalf of another actor named in the graph (the *delegator*). This is a declarative designation over two declared actors; it adds no node class and no cord, riding the existing authority cord as an attribute.

**No-amplification invariant (well-formedness).** The delegate's granted `risk` set over a `kind` at a gate MUST be a subset of the delegator's over that `kind` at that gate. Delegation MUST NOT amplify granted authority [OCAP; PROT]. The same attenuation applies to grade: a delegate's granted grade MUST NOT be above the delegator's on the active ladder; if the delegator is ungraded, the delegate MUST also be ungraded. The check ranges over declared grants only. It does not propagate the delegator's reservation, quorum, or prohibition to the delegate (policy, §10). A graph violating this invariant is ill-formed and has no effect.
...
Attachment (not discharge, §6/§10) is the in-language condition; discharge is out of scope. Within this specification only `auto` releases: `human` and `reserved` denote withhold-pending-intervention. The recorded reason for a withholding SHOULD be the most restrictive contributing verdict.
...
- **Node attributes** — a node's configuration is projected on the node, never as a separate member: a `gate`'s `risk_floor` and `party`, a `human`'s `role`, an `actor`'s granted `grade`, and a source gate's required `grade_required`.
...
| grade attenuation under delegation | granted grade MUST NOT be amplified by delegation, pairwise [OCAP; PROT]; bounds delegated autonomy only (§6) |
